...
cd "$ROOT" 2>/dev/null || { echo "FAIL: $ROOT does not exist — run run-real-story.sh first"; exit 99; }
...
if [[ -n "$ROOT_TASK_ID" ]]; then
  ...
  done_count=$(hermes kanban list --tenant KanbanReal --status done --json 2>/dev/null | python3 -c "
import json, sys
try:
    d = json.loads(sys.stdin.read())
...
LANDER_TASK_ID=$(hermes kanban list --tenant KanbanReal --status done --json 2>/dev/null | python3 -c "
import json, sys
try:
    d = json.loads(sys.stdin.read())
