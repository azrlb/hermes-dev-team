...
ROOT=/tmp/hermes-kanban-real-story
rm -rf "$ROOT"
...
## Architecture
- TypeScript (ESM), vitest, beads (prefix KanbanReal).
...
cat > bin/pi <<'SH'
#!/usr/bin/env bash
LOGDIR=/tmp/hermes-kanban-real-story/.hermes
mkdir -p "$LOGDIR" /tmp/hermes-kanban-real-story/.hermes/sessions
...
# ─── NO bin/hermes shim (real-story test) ────────────────────────────────────
# This fixture is the moment-of-truth test: dispatcher spawns workers via
# the real `hermes -p <profile> ...`, which actually invokes MiMo via Nous
# Portal to do the kanban-worker LLM turns. The bin/pi shim is still in
# place so Pi (the inner subprocess) writes the impl deterministically and
# returns APPROVED for the lander's Quinn review — what we're testing here
# is the cloud<->kanban round-trip, not local Pi.
...
BARE_REMOTE=/tmp/hermes-kanban-real-story-remote.git
rm -rf "$BARE_REMOTE"
...
echo "[setup] bd init..."
bd init --prefix KanbanReal >/dev/null 2>&1 || true
...
budget_usd=2.00" 2>&1)
echo "$BD_OUT"
STORY_ID=$(echo "$BD_OUT" | grep -oE 'KanbanReal-[a-z0-9]+' | head -1)
if [[ -z "$STORY_ID" ]]; then
  STORY_ID=$(bd list --json 2>/dev/null | grep -oE '"id"[[:space:]]*:[[:space:]]*"KanbanReal-[a-z0-9]+"' | head -1 | grep -oE 'KanbanReal-[a-z0-9]+')
fi
...
export HERMES_TENANT=KanbanReal
...
echo "[run] REAL-STORY mode: cloud workers (MiMo via Nous Portal) actually run"
echo "[run] waiting for dispatcher to drain (max 30 minutes — cloud LLM workers)"
...
" 2>/dev/null)
  ...
  graph=$(hermes kanban list --tenant KanbanReal --json 2>/dev/null | python3 -c "
import json, sys
try:
    d = json.loads(sys.stdin.read())
