...
kanban_heartbeat(
    ...
    note="running tsc --noEmit, awaiting result",
)
```

Good notes name what's happening (`"3 of 7 lint errors fixed"`, `"installing missing @types/node"`). Bad notes: `"still working"`, empty, or sub-second intervals. Skip heartbeats only if the whole run will finish in under 2 minutes.
