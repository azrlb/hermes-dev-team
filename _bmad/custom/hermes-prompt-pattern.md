...
> **⚠ DEPRECATED for overnight work (2026-06-07).** Overnight execution now runs
> via the **beads cron drain** (FlowInCash-Core 6pm, LivingApp-Sidecar 8pm,
> Crispi-app 10pm): the drain reads each ready bead's **description + notes**
> directly and classifies it for execution — **the bead IS the prompt**. Do NOT
> hand-fire `hermes chat` kickoff commands for overnight queues, and do NOT
> paste kickoff commands into bead notes (stale commands confuse triage).
> Author drain-conformant beads instead: what to do (file paths), acceptance
> criteria, test command, "CLEAR PLAN EXISTS" for multi-file work, and no
> unintended skip-trigger phrases ("Needs decision", "needs UX spec",
> "post-beta", …).
> **Canonical procedure:** `skills/dev-team/vibe-loop/references/overnight-backlog-drain.md`
> (multi-project: `references/overflow-drain-triage.md`).
> The templates below remain valid ONLY for interactive daytime one-offs where
> a human fires the command and watches it run.

**How to create prompts that work with Hermes Agent pipelines.**

*Last updated: 2026-06-07 (deprecation banner); pattern content 2026-05-14*
